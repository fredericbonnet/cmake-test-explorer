...
export class CmakeAdapter implements TestAdapter {
	...
	private async loadTestSuite() {
		try {
				...
				for (let test of this.cmakeTests) {
					...
					for (let name of path) {
						...
						let childSuite = suite.children.find(
							(item) =>
								item.type == 'suite' &&
								item.id === currentId + SUITE_SUFFIX
						);
						if (!childSuite) {
							childSuite = {
								type: 'suite',
			...
			if (
				e instanceof CacheNotFoundError &&
				!(await isCmakeWorkspace())
			) {
				...
				(event: CmakeTestEvent) => {
					switch (event.type) {
						...
						case 'output':
							...
							if (matches?.groups) {
								const { file, line, severity, message } =
									matches.groups;
	...
	private async getRunOptions(): Promise<CmakeTestRunOptions> {
		...
		const extraCtestEnvVars =
			await this.getConfigObject('extraCtestEnvVars');
			...
			if (suiteDelimiter && id.endsWith(suiteDelimiter + SUITE_SUFFIX)) {
				...
				tests.push(
					...allTests.filter((test) =>
						test.startsWith(
							id.substr(0, id.length - SUITE_SUFFIX.length)
						)
					)
				);
			} else {
